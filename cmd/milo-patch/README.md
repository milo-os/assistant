# milo-patch — the assistant as a datumctl plugin

Makes Patch reachable as `datumctl patch`, so the assistant is one verb inside
...
registers is still `patch`; the prefix names the binary, not the command.
...
```bash
task build:plugin                       # → ./bin/milo-patch
...
`INSTALL=1` copies the binary to `~/.datumctl/plugins/datumctl-patch` — a
...
- In its **managed directory** it resolves the verb `patch` by trying `patch`
  then `datumctl-patch`, and never `milo-patch`. A generic `patch` is re-hashed
...
- On **`$PATH`** it tries `datumctl-patch` then `milo-patch`, so the shipped
...
```bash
cp ./bin/milo-patch /usr/local/bin/    # anywhere on $PATH
export DATUMCTL_TRUSTED_PLUGINS=patch  # keyed on the verb, not the filename
```

Without one of those, `datumctl patch` fails with *"unmanaged plugin that has
...
says and extraction prefers a `milo-patch` member over `datumctl-patch` over
bare `patch` — the shipped name works and the archive filename is free. Via
...
`datumctl-patch_Darwin_arm64.tar.gz`. Supporting both means publishing under the
`datumctl-` asset name while keeping `milo-patch` as the member inside.
