...
func newAgentRunner(ctx context.Context, cfg *config.Config, log *slog.Logger, metrics *appmetrics.Metrics) (assistanta2a.AgentRunner, history.Store, func(), error) {
	...
	if cfg.ConversationStoreURL != "" {
		...
		// Chained, never assigned. `cleanup = pg.Close` would silently drop
		// whatever is already in the chain — the status writer is registered
		// above, so a plain assignment here leaks its worker and abandons its
		// queued writes in exactly the configuration production runs (durable
		// store set). The two stores below already chain; this one did not.
		store = pg
		prevCleanup := cleanup
