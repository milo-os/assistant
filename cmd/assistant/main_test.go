...
func TestAssistantBootsAndServesHealthAndCard(t *testing.T) {
	...
	cmd.Env = append(os.Environ(),
		"PORT="+port,
		"HOST=127.0.0.1",
		// Identity and access are answered by the control plane. This boot
		// test never issues an authenticated request — it checks /healthz and
		// the public agent card — so the endpoint only has to be well-formed,
		// not reachable.
