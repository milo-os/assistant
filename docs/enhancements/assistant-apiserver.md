...
- **`Conversation`** — namespaced; **namespace == milo project** (maps to the existing `project_name` column). `metadata.name = context_id`, `creationTimestamp = created_at`. `status: { lastActiveAt, messageCount, title, name }` (`title` is the opening user message, one line, truncated — what the conversation is about, so a picker can show and search it without fetching every transcript; `name` is what the user chose to call it, set through the service and shown in place of the title where present). No spec (born from chat).
...
- `patch conversations list --project <p>` — table (context-id, created, last-active, message count). `patch conversations show <context-id> --project <p>` — full transcript. Both fetch **raw API paths** and deserialize into the local `pkg/apis/assistant/v1alpha1` types (pure apimachinery, already a dep) — no client-go, and no client-side discovery to go stale. Under `datumctl assistant` the transport is a plain HTTPS GET carrying datumctl's own token, so the read views use the **same identity and the same project** as `chat`; the standalone `patch` binary, and anyone passing `--kubeconfig`, falls back to `kubectl get --raw`. An earlier version used only the kubectl path on the reasoning that an apiserver read deserves the caller's k8s identity (decision #7); Milo accepts datumctl's token, so that split bought no second identity, only a second way to address the wrong cluster. Code: `internal/patchcli/conversations.go`.
