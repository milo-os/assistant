...
| `PLAN_TOKEN_KEY` | generated per process | Secret the base tools' change path binds a plan with (base64, or a raw string of at least 16 bytes). A plan token is a hash of exactly what a person was shown, and `resources_apply` refuses anything that does not re-derive to it, so this key is what lets a plan made by one replica be applied by another and survive a restart. Unset generates one per process **with a startup warning**: the guarantee still holds, but an outstanding plan is lost on restart and refused by a sibling replica, and the person is asked to plan again |
...
| `PLATFORM_CAPABILITY_DOCS_FIXTURE` | — | Path to a JSON file of **platform** capability documents — composed into **every** project regardless of entitlement, because the services publishing them are platform infrastructure rather than catalog services. Mutually exclusive with `PLATFORM_CAPABILITY_PROVIDER_URL`, and deliberately **not** exclusive with the two settings above: a full deployment sets one of each. `serviceAgentRef` and `configurationVersion` are optional on this path (catalog-only fields); `serviceRef.name` and `serviceName` stay required, and `metadata.namespace` is cleared so every project keeps the document. See [Platform capabilities](capability-reference.md#platform-capabilities-every-project-no-entitlement) |
| `PLATFORM_CAPABILITY_PROVIDER_URL` | — | Base URL of an HTTP provider serving platform capability documents at the same endpoint the per-project source uses; mutually exclusive with `PLATFORM_CAPABILITY_DOCS_FIXTURE`. Both unset ⇒ no capability reaches a project without an entitlement |
| `CAPABILITY_UNMETERED_SERVICES` | — | Comma-separated `serviceName`s whose tool calls emit **no** `tool-invocations` billing event. Every platform document's service is already unmetered without being listed here — a project did not choose a platform capability, so it is not billed for one — and this **extends** that set rather than replacing it; a platform service cannot be put back on the meter. Unmetered means no event at all, never an event on a different meter. See [Metering](architecture/metering.md) |
