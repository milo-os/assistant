...
// exercise the middleware's use of the [auth.Authenticator] seam — that a
// missing or unknown token is a 401, and that the resolved subject reaches the
// authorizer — none of which depends on how identity is really established.
// The live TokenReview path is covered in internal/auth against a fake
// reviewer, so nothing here needs a control plane.
type stubAuthenticator map[string]string

func (s stubAuthenticator) Authenticate(_ context.Context, token string) (auth.Principal, error) {
	subject, ok := s[token]
	if !ok {
		return auth.Principal{}, auth.Unauthenticated("Unknown or invalid bearer token")
	}
	return auth.Principal{Subject: subject}, nil
}

// stubAuthorizer allows a fixed subject→projects map, mirroring the decisions a
// SubjectAccessReview would return, so the middleware's 403 paths stay covered
// without a control plane.
type stubAuthorizer map[string][]string

func (s stubAuthorizer) AuthorizeProject(_ context.Context, p auth.Principal, projectName string) error {
	if slices.Contains(s[p.Subject], projectName) {
		return nil
	}
	return auth.Unauthorized("Subject may not act on project " + projectName)
}

// testAuth returns the identity and access decisions the server tests assume:
// alice may act on the test project, bob only on another one.
func testAuth() (stubAuthenticator, stubAuthorizer) {
	return stubAuthenticator{goodToken: "alice", wrongToken: "bob"},
		stubAuthorizer{"alice": {project}, "bob": {"other-project"}}
}
