...
// deny (allowed=false) → 403.
//
// This once also asserted that a principal claiming every project ("*") could
// not talk its way past the SAR. A Principal can no longer carry grants at all,
// so that escalation is unrepresentable rather than merely rejected — the
// control plane is the only source of an allow.
