...
func TestNewRunner_PersonaPromptFileMissing(t *testing.T) {
	cfg := loadTestConfig(t, map[string]string{"PERSONA_PROMPT_FILE": filepath.Join(t.TempDir(), "missing.md")})
	_, _, _, err := NewRunner(context.Background(), cfg, slog.New(slog.DiscardHandler), appmetrics.New())
	if err == nil {
		t.Fatal("want error for missing persona prompt file, got nil")
	}
}

func TestNewRunner_PersonaPromptFileRead(t *testing.T) {
	...
	_, _, cleanup, err := NewRunner(context.Background(), cfg, slog.New(slog.DiscardHandler), appmetrics.New())
	if err != nil {
...
// rather than failing boot. cleanup() is what stops the status writer's worker,
// so this also covers the goroutine both binaries depend on being stopped.
func TestNewRunner_CRDSourceBoots(t *testing.T) {
