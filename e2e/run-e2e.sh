...
# control-plane auth). Set ASSISTANT_NO_BOOT=1 to test an already-running service.
set -euo pipefail
...
# Token values are minted from the cluster below (see the control-plane block;
...
# Control-plane auth. There is no static token table any more: the assistant
# resolves callers with a TokenReview and authorizes them with a
# SubjectAccessReview, so this harness points a locally-booted binary at the
# kind cluster's apiserver and mints real ServiceAccount tokens. The local run
# therefore exercises the same auth path a deployed pod does.
#
# GOOD_TOKEN  -> patch-dev, bound to create conversations (allowed)
# WRONGPROJ_TOKEN -> patch-dev-unauthorized, authenticates but is bound to
#                    nothing, so every project is a 403. It replaces the old
#                    "granted a different project" token; the denial now comes
#                    from the control plane rather than a local grant list.
CP_API_URL="${CP_API_URL:-$(kubectl config view --minify -o jsonpath='{.clusters[0].cluster.server}')}"
CP_CA_FILE="${OUT}/control-plane-ca.crt"
kubectl config view --raw --minify -o jsonpath='{.clusters[0].cluster.certificate-authority-data}' \
  | base64 -d >"${CP_CA_FILE}" 2>/dev/null || : >"${CP_CA_FILE}"
# The assistant's own identity for issuing reviews (system:auth-delegator).
ASSISTANT_SA_TOKEN_FILE="${OUT}/assistant-sa.token"
kubectl -n patch-playground create token assistant --duration=1h >"${ASSISTANT_SA_TOKEN_FILE}"
GOOD_TOKEN="${GOOD_TOKEN:-$(kubectl -n patch-playground create token patch-dev --duration=1h)}"
WRONGPROJ_TOKEN="${WRONGPROJ_TOKEN:-$(kubectl -n patch-playground create token patch-dev-unauthorized --duration=1h)}"

# Env every locally-booted assistant needs to reach the control plane.
CONTROL_PLANE_ENV=(
  "AUTHN_TOKENREVIEW_API_URL=${CP_API_URL}"
  "AUTHZ_SAR_API_URL=${CP_API_URL}"
  "AUTHN_TOKENREVIEW_TOKEN_PATH=${ASSISTANT_SA_TOKEN_FILE}"
  "AUTHZ_SAR_TOKEN_PATH=${ASSISTANT_SA_TOKEN_FILE}"
  "AUTHN_TOKENREVIEW_CA_CERT_PATH=${CP_CA_FILE}"
  "AUTHZ_SAR_CA_CERT_PATH=${CP_CA_FILE}"
)
