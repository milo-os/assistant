...
| `AUTH_DEV_TOKENS` | `e2e-token:e2e-user:demo-project;wrong-token:other:other-project` | dev token table passed to the service |
...
- **Boot command** `bun run start` (= `bun run src/index.ts`); env vars `PORT`,
  `AUTH_MODE`, `AUTH_DEV_TOKENS`, `AGENT_BINDINGS_FIXTURE`, `MODEL_MODE`,
  `USAGE_GATEWAY_URL`, `PUBLIC_BASE_URL`.
- **`AUTH_DEV_TOKENS` format** `token:subject:projA,projB;...` (`src/auth/dev.ts`):
  `;`-separated entries, three `:`-fields, comma-separated projects, `*` grants all.
  Unknown token → 401; known token not granting the project → 403.
